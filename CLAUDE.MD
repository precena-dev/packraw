...
本プロジェクトでは、GitHub Actionsを使用した自動リリースフローを採用しています。

**リリース手順**:
1. コード修正とコミット
2. バージョン番号を更新（package.jsonの`version`フィールド）
3. Gitタグを作成してプッシュ
   ```bash
   git tag v2.5.0
   git push origin v2.5.0
   ```
4. GitHub Actionsが自動的に実行され、以下を実施:
   - macOS/Windows/Linux向けビルド
   - 配布パッケージの作成（.dmg, .zip, .exe等）
   - latest.yml等の更新情報ファイル生成
   - GitHub Releasesへの自動アップロード

**バージョニング規則（Semantic Versioning）**:
- **Major（v3.0.0）**: 破壊的変更を含むアップデート
- **Minor（v2.5.0）**: 後方互換性のある機能追加
- **Patch（v2.5.1）**: 後方互換性のあるバグ修正

#### パッチ（バグ修正）リリース手順

バグ修正のパッチリリースは以下の手順で実施します:
...
2. **修正をコミット**
   ```bash
   git add .
   git commit -m "fix: バグの説明（簡潔に）"
   # 例: git commit -m "fix: 土日の自動出勤を無効化"
   ```

3. **バージョン更新（自動コミット＋タグ作成）**
   ```bash
   npm version patch
   # 例: 2.5.0 → 2.5.1
   # 以下が自動的に実行される:
...
5. **自動ビルド・リリース**
   ...
   - 約10-15分でビルドが完了
   - GitHub Releasesにパッケージが公開される

6. **ユーザーへの配布**
   - electron-updaterが24時間以内に新バージョンを検出
   - ユーザーのアプリに更新通知が表示される
   - ユーザーが更新を承認すると自動ダウンロード・インストール

#### 緊急パッチの場合

重大なバグやセキュリティ問題の場合:

1. **Hotfixブランチ作成（オプション）**
   ```bash
...
- [ ] GitHub Actionsのビルドが成功したか確認
- [ ] GitHub Releasesにファイルがアップロードされたか確認
- [ ] リリースノートを編集（自動生成されたものを整形）
- [ ] 重要な変更の場合はSlackで通知

#### 注意事項

- **タグの命名規則**: 必ず `v` プレフィックスを付ける（例: `v2.5.1`）
- **手動でのタグ作成は避ける**: `npm version` コマンドを使用して自動作成
- **タグの削除**: 誤ったタグをプッシュした場合は、すぐに削除して再作成
  ```bash
  git tag -d v2.5.1                    # ローカルのタグ削除
  git push origin :refs/tags/v2.5.1   # リモートのタグ削除
  ```
- **GitHub Releasesの削除**: 誤ったリリースはGitHub UIから手動で削除可能
...
- **セクション2: 土日制御**
  - 土日の打刻を無効化（トグル）
  - 当日の自動打刻・編集を制限（過去記録は編集可能）

- **セクション3: 時間帯別自動退勤**
  - 指定時刻以降のスリープ自動退勤（トグル）
  - 自動退勤開始時刻設定（時刻入力）
  - デフォルト17:00以降のスリープ/サスペンド時に自動退勤
...
- **セクション1: 休憩打刻予約**
  ...
  - 休憩開始時刻設定（デフォルト12:00）
  - 休憩終了時刻設定（デフォルト13:00）
  - ランダム誤差設定（0-30分、デフォルト±5分）
  - 設定時刻の前後にランダムなタイミングで自動打刻

- **セクション2: PC連動自動休憩モード**
  - 自動休憩モード（トグル）
  - PowerMonitorによる画面ロック/サスペンド検知
  - トリガー説明表示
