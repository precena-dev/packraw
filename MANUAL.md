...
- **Windows（インストーラー版・推奨）**: `PackRaw-Setup-x.x.x.exe`
  - デフォルトインストール先: `C:\Users\{ユーザー名}\AppData\Local\Programs\PackRaw`
...
設定画面は以下の3つのタブで構成されています：
- **出退勤タブ**: 自動出退勤、土日制御、時間帯別自動退勤、スタートアップ
- **休憩タブ**: 休憩打刻予約、PC連動自動休憩モード
- **その他タブ**: 自動更新、アプリ情報

### 設定画面の開き方

1. **メイン画面右上の⚙️アイコンをクリック**
2. **設定モーダルが開きます**
3. **タブをクリックして切り替え**
   - 「出退勤」「休憩」「その他」の3タブ
...
- 自動退勤は勤務中の場合のみ動作します。またアプリが起動してないと退勤できません。
...
**動作内容**:
- **有効時**: 土日の当日の自動打刻・手動打刻・編集が制限されます
- **過去記録の編集**: 過去の土日の記録は編集可能（休日出勤の修正に対応）
- **デフォルト**: 有効（ONの状態）

**効果**:
- 休日の誤打刻を防止
- 休日出勤の場合は設定をOFFにして対応可能

#### セクション3: 時間帯別自動退勤

指定時刻以降のスリープ・サスペンド時に自動退勤する機能です。

**設定項目**:
- **指定時刻以降のスリープ自動退勤**（トグルスイッチ）
- **自動退勤開始時刻**（時刻入力、デフォルト: 17:00）

**動作内容**:
- 設定した時刻以降にPCをスリープ・サスペンドすると自動的に退勤打刻
- 休憩中の場合は、自動的に休憩終了してから退勤
- 既に退勤している場合は何もしません
- シャットダウン時は既存の「PCシャットダウン時自動退勤」設定で制御

**動作例**:
- 設定: 17:00以降に自動退勤
- 16:50にPCをスリープ → 何もしない
- 17:10にPCをスリープ → 退勤打刻（休憩中なら先に休憩終了）

**⚠️ 注意**:
- スリープ・サスペンド時のみ動作（シャットダウンは別設定）
- 土日制御が有効な場合、土日は動作しません

#### セクション4: スタートアップ

PCログイン時にアプリを自動起動する機能です。

**設定項目**:
- **ログイン時に自動起動**（トグルスイッチ）

**動作内容**:
- PCにログインすると自動的にPackRawが起動します
- macOS/Windows対応
- Linuxは非対応（設定画面にメッセージ表示）

**設定確認方法**:
- **macOS**: システム環境設定 > ユーザとグループ > ログイン項目
- **Windows**: タスクマネージャー > スタートアップ
...
**設定項目**:
- **自動休憩打刻**（トグルスイッチ）
- **休憩開始時刻**（時刻入力、デフォルト: 12:00）
- **休憩終了時刻**（時刻入力、デフォルト: 13:00）
- **ランダム誤差**（0-30分、デフォルト: ±5分）
...
PCのシステムイベント（画面ロック、スリープ）を検知し、自動休憩を行う機能です。
...
**トリガー**:
- 休憩開始: 画面ロック / システムサスペンド時
- 休憩終了: 画面アンロック / システムレジューム時
...
**⚠️ 注意**:
- 時間帯別自動退勤が有効な場合、指定時刻以降は自動退勤が優先されます
- 土日制御が有効な場合、土日は動作しません
...
- アプリ起動後30秒後に初回チェック
- その後6時間ごとに定期チェック
- 新バージョン利用可能時に通知ダイアログ表示
- **macOS**: 手動更新（ダウンロードリンク提供）
- **Windows**: 自動ダウンロード・インストール対応
...
PackRawのバージョンと設定ファイル情報を表示します。

**表示項目**:
- **バージョン**: 現在のアプリバージョン（例: v2.5.0）
- **設定ファイルパス**: freee-app-config.jsonの保存場所
  - **macOS**: `~/Library/Application Support/PackRaw/freee-app-config.json`
  - **Windows**: `C:\Users\{ユーザー名}\AppData\Roaming\PackRaw\freee-app-config.json`

**用途**:
- バージョン確認
- 設定ファイルの場所確認（トラブルシューティング時）
- サポート依頼時の情報提供
